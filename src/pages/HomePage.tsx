...
import { Sparkles, ArrowRight, Brain, BookOpen, BarChart3, Zap, CheckCircle, HelpCircle } from 'lucide-react';
import { SeoHead } from '../components/SeoHead';

const features = [
    { icon: Brain, title: 'Generación con IA', description: 'Crea tests automáticamente a partir de tus apuntes. Solo pega tu temario y la IA genera preguntas de opción múltiple, verdadero/falso y más.' },
    { icon: BookOpen, title: 'Múltiples formatos', description: 'Importa desde texto, PDF o crea preguntas manualmente. Soporta formatos GIFT estándar compatibles con Moodle y otras plataformas.' },
    { icon: BarChart3, title: 'Estadísticas detalladas', description: 'Seguimiento de tu progreso por asignatura y tema. Identifica tus puntos débiles y mide tu mejora a lo largo del tiempo.' },
    { icon: Zap, title: 'Práctica adaptativa', description: 'Dificultad configurable (fácil, medio, difícil). El sistema aprende de tus respuestas para enfocarse en lo que más necesitas repasar.' }
];
...
export default function HomePage() {
    ...
    return (
        <>
            <SeoHead
                title="FlashTests — Generador de tests con IA para estudiar"
                description="FlashTests te ayuda a crear tests de autoevaluación con IA a partir de tus apuntes. Estudia más rápido, repite menos y retiene más. Plan gratuito disponible."
                path="/"
            />
            ...
            <div className="space-y-20 pb-20">
                <section className="text-center pt-8 pb-4">
                    ...
                    <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 dark:text-white leading-tight mb-6">
                        ...
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600">con tests inteligentes</span>
                    </h1>
                    <p className="text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto mb-10">
                        FlashTests genera tests de autoevaluación a partir de tus apuntes usando IA. Identifica lo que no sabes, repasa menos y retiene más.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <button onClick={() => navigate('/upload')} className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-indigo-500/30 text-lg flex items-center justify-center gap-2">
                            Empezar Gratis <ArrowRight className="w-5 h-5" />
                        </button>
                        <button onClick={() => navigate('/login')} className="px-8 py-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold rounded-xl border border-slate-200 dark:border-slate-600 transition-colors text-lg">
                            Ya tengo cuenta
                        </button>
                    </div>
                </section>
                ...
                <section>
                    <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-900 dark:text-white mb-12">¿Cómo funciona?</h2>
                ...
                <section>
                    <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-900 dark:text-white mb-4">Todo lo que necesitas para estudiar</h2>
                    <p className="text-center text-slate-600 dark:text-slate-400 max-w-xl mx-auto mb-12">Herramientas diseñadas para estudiantes que quieren optimizar su tiempo de estudio</p>
                ...
                <section className="bg-slate-50 dark:bg-slate-800/50 rounded-3xl p-8 sm:p-12 max-w-5xl mx-auto border border-slate-200 dark:border-slate-700">
                    <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-900 dark:text-white mb-4">Planes para cada necesidad</h2>
                    <p className="text-center text-slate-600 dark:text-slate-400 max-w-xl mx-auto mb-10">Empieza gratis y escala cuando lo necesites</p>
                ...
                <section>
                    <h2 className="text-2xl sm:text-3xl font-bold text-center text-slate-900 dark:text-white mb-12">Preguntas frecuentes</h2>
                    <div className="max-w-2xl mx-auto space-y-3">
                        {faqs.map((faq, i) => (
                            <div key={i} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full p-4 text-left flex items-center justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                ...
                <section className="text-center bg-gradient-to-br from-indigo-500 to-purple-600 rounded-3xl p-10 sm:p-16 max-w-4xl mx-auto">
                    <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">¿Listo para estudiar de forma más inteligente?</h2>
                    <p className="text-indigo-100 max-w-xl mx-auto mb-8 text-lg">Únete a miles de estudiantes que ya usan FlashTests para mejorar sus resultados.</p>
                    <button onClick={() => navigate('/upload')} className="px-8 py-4 bg-white text-indigo-600 font-bold rounded-xl hover:bg-indigo-50 transition-colors shadow-lg text-lg">
